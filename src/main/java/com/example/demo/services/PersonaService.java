...
@Transactional
public PersonaDTO findById(int id) throws PersonaIdNotFoundException {
	
	if(repository.existsById(id)) {
		Persona persona = repository.findById(id).get();
		PersonaDTO unDto = new PersonaDTO();
		
		unDto.setId(persona.getId());
		unDto.setNombre(persona.getNombre());
		unDto.setApellido(persona.getApellido());
		unDto.setDni(persona.getDni());
		
		return unDto ;
	}else {
		throw new PersonaIdNotFoundException("ID: "+id);
	}
